...
export declare function getInteractiveGoals(rs: RpcSessionAtPos, pos: TextDocumentPositionParams): Promise<InteractiveGoals | undefined>;
export declare function getInteractiveTermGoal(rs: RpcSessionAtPos, pos: TextDocumentPositionParams): Promise<InteractiveTermGoal | undefined>;
export type Name = string;
export type StrictOrLazy<S, L> = {
    strict: S;
} | {
    lazy: L;
};
...
export type MsgEmbed = {
    ...
    lazyTrace: [number, Name, MessageData];
};
export type InteractiveDiagnostic = Omit<LeanDiagnostic, 'message'> & {
    message: TaggedText<MsgEmbed>;
};
export interface LineRange {
    start: number;
    end: number;
}
export declare function getInteractiveDiagnostics(rs: RpcSessionAtPos, lineRange?: LineRange): Promise<InteractiveDiagnostic[]>;
export declare function InteractiveDiagnostics_msgToInteractive(rs: RpcSessionAtPos, msg: MessageData, indent: number): Promise<TaggedText<MsgEmbed>>;
export declare function lazyTraceChildrenToInteractive(rs: RpcSessionAtPos, children: LazyTraceChildren): Promise<TaggedText<MsgEmbed>[]>;
export declare function InteractiveDiagnostics_infoToInteractive(rs: RpcSessionAtPos, info: InfoWithCtx): Promise<InfoPopup>;
export type GoToKind = 'declaration' | 'definition' | 'type';
export declare function getGoToLocation(rs: RpcSessionAtPos, kind: GoToKind, info: InfoWithCtx): Promise<LocationLink[]>;
...
export declare function Widget_getWidgets(rs: RpcSessionAtPos, pos: Position): Promise<UserWidgets>;
 ...
 */
export declare function Widget_getWidgetSource(rs: RpcSessionAtPos, pos: Position, hash: string): Promise<WidgetSource>;
